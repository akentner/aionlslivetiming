...
**Core Value:** Downstream Python projects can subscribe to a running NLS race and get typed, filtered, cached race data (positions, lap times, sector times, pit messages, statistics) — live or replayed from a log — without ever touching the Azure WebSocket or the cryptic short-code JSON the server actually emits.
